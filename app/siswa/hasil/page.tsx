...
export default function SiswaHasil() {
  ...
  useEffect(() => {
    const fetchLatestResult = async () => {
      try {
        // First, get the latest test ID from history
        const historyRes = await fetch("/api/siswa/riwayat")
        if (!historyRes.ok) {
          throw new Error("Failed to fetch history")
        }
        const historyData = await historyRes.json()
        const latestTest = historyData.history.find((h: any) => h.status === "selesai")
        if (!latestTest) {
          setError("Belum ada hasil tes yang selesai")
          setLoading(false)
          return
        }

        // Then, fetch the result for the latest test
        const resultRes = await fetch(`/api/siswa/hasil/${latestTest.id}`)
        if (!resultRes.ok) {
